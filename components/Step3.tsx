...
export default function Step3({ onNext, onBack, initialData, userType }: Step3Props) {
  ...
  // VALIDERING: Tjekker om ALLE felter (Navn, Køn og fuld Fødselsdag) er udfyldt
  const isInvalid = !name || !gender || !birthday.day || !birthday.month || !birthday.year;

  // FUNKTION: Pakker alle indtastede data sammen og sender dem videre til næste trin
  const handleNext = () => {
    if (isInvalid) return; // Ekstra sikkerhed: stop hvis noget mangler
    onNext({ gender, name, birthday });
  };

  const handleNumberChange = (field: string, value: string) => {
    const onlyNumbers = value.replace(/[^0-9]/g, '');
    setBirthday({ ...birthday, [field]: onlyNumbers });
  };
    ...
    <div className="flex flex-col items-center animate-in fade-in slide-in-from-bottom-4 duration-500 w-full pb-20 text-center">
      <div className="max-w-2xl w-full">
        ...
        <div className="bg-white p-6 border-2 border-gray-100 shadow-sm text-left space-y-6 max-w-md mx-auto mb-12 rounded-none">
          ...
          <div className="space-y-2">
            <label className="block text-navy font-bold text-lg uppercase font-kbh">
              {userType === "self" ? "Ваше ім'я:" : "Ім'я дитини:"} <span className="text-gray-400">*</span>{" "}
          ...
          <div className="space-y-3">
            <label className="block text-navy font-bold text-lg uppercase font-kbh">
              Стать: <span className="text-gray-400">*</span> <span className="text-navy/60 font-normal normal-case italic font-kbhtekst text-sm ml-1">(Køn)</span>
            </label>
          ...
          <div className="space-y-2">
            <label className="block text-navy font-bold text-lg uppercase font-kbh">
              день народження: <span className="text-gray-400">*</span> <span className="text-navy/60 font-normal normal-case italic font-kbhtekst text-sm ml-1">(Fødselsdag)</span>
            </label>
        ...
        <div className="flex items-center justify-between w-full max-w-md mx-auto px-2">
          ...
          {/* OPRETTER DYNAMISK KNAP DER TYDELIGT VISER AT NOGET MANGLER */}
          <Button 
            variant="purple" 
            size="kk" 
            disabled={isInvalid}
