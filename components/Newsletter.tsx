...
export default function Newsletter() {
  ...
  // Dynamisk formSchema flyttet herind, så Zod-fejlbeskeden reagerer øjeblikkeligt på sprogskift
  const formSchema = z.object({
    email: z
      ...
      // Sørger for at transformere input til små bogstaver, så casing er ligegyldig
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    // values.email er nu automatisk i lowercase pga. Zod transform
    const { error } = await supabase
      .from('newsletter')
      .insert([{ email: values.email }])

    if (error) {
      console.warn("Supabase fejl:", error.code, error.message)
    ...
    <section className="w-full bg-primary-blue py-6">
      <div className="mt-10 px-8 md:px-6 flex flex-col items-center text-center">
        ...
        <div className="w-full">
          <h2 className="text-navy text-3xl font-bold md:text-3xl mb-2 font-kbh">Інформаційний бюлетень</h2>
          <p className="text-navy text-xl md:text-2xl font-normal">Nyhedsbrev</p>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex-1 space-y-0 text-left w-full">
                  ...
                  <FormMessage className="text-red-600 text-center absolute left-0 right-0 mt-20 md:mt-4 text-xs font-bold" />
                </FormItem>
              )}
            />
        ...
        <div className="h-8 mt-4 w-full"> 
          ...
          {status === "duplicate" && (
            <p className="text-red-600 font-bold animate-in fade-in slide-in-from-top-1 text-sm">
              {t.duplicateMsg}
            </p>
          )}
          {status === "error" && (
