...
// DATA: Liste over alle aktiviteter
const activitiesData = [
  ...
  { 
    id: 3, 
    title: 'Танці | Dans', 
    image: '/image/hero-dans.jpg object-center', 
  ...
  { 
    ...
    ua: 'Елегантний та динамічний спорт, що розвиває блискавичну реакцію, координацію та strategicчне мислення. Тренування включають роботу зі зброєю (рапіра, шпага або шабля) та вивчення техніки швидких випадів і захисту.',
  ...
  { 
    ...
    image: '/image/badminton-2.jpg object-right', 
  ...
  { 
    id: 16, 
    title: 'Скаути | Spejder', 
    image: '/image/spejder.jpg object-center', 
    ua: 'Розвиток самостійності, командної роботи та лідерства через активний відпочинок на природі, табори, орієнтування та вивчення скаутських традицій.',
    dk: 'Udvikling af selvstændighed, samarbejde og lederskab gennem friluftsliv, lejrture, kortlægning, knob og rige spejdertraditioner i naturen.'
  }
];

export function ActivityOverview() {
  // HOOKS: Navigation, sprog og søgestatistik
  const [searchQuery, setSearchQuery] = useState("");
  const router = useRouter();
  const { language } = useLanguage();

  // LOGIK: Filtrering baseret på søgefeltet
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-0 md:px-0 bg-white">
      ...
      {/* HEADER SEKTION: Blå boks med titel og info */}
      <div className="relative w-full max-w-4xl mx-auto h-40 md:h-50 overflow-hidden -mt-20 md:mt-0  bg-primary-blue font-kbh text-navy">
        <div className="absolute inset-0 flex flex-col justify-center p-6 md:p-10 z-10">
          ...
          <p className="text-sm md:text-base font-kbhtekst max-w-2xl leading-tight opacity-90">
            {language === "ua"
              ? "Приєднайтесь до наших цікавих занять! Усі проекту створені для зміцнення спільноти та активного відпочинку."
       ...
       {/* FOOTER NAVIGATION: Tilbage-knap */}
      <div className="flex justify-start w-full max-w-6xl mx-auto py-6 px-4 md:px-0">
        <button 
          onClick={() => router.push('/')} 
      ...
      <div className="w-full max-w-xl mx-auto mb-12 md:mb-12 px-4 md:px-0">
        <div className="flex flex-col md:flex-row items-stretch w-full gap-3">
          ...
          {/* Inputfelt med søgeikon og slet-knap */}
      ...
      {/* GRID SEKTION: Visning af kortene */}
      {filteredActivities.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-x-8 md:gap-y-12 w-full max-w-6xl mx-auto px-4 md:px-0">
          {filteredActivities.map((activity) => {
            // Vælger sprog baseret på sprog-kontekst
            const displayDescription = language === "ua" ? activity.ua : activity.dk;
            
            // Deler billed-URL og den individuelle Tailwind object-position fra hinanden
        ...
        /* Hvis søgningen ikke matcher nogen aktiviteter */
      ...
      {/* FOOTER NAVIGATION: Tilbage-knap */}
      <div className="flex justify-start w-full max-w-6xl mx-auto mt-12 py-6 px-4 md:px-0">
        <button 
          onClick={() => router.push('/')} 
          className="flex items-center gap-2 text-black text-lg font-bold hover:text-secondary-purple transition-all group cursor-pointer"
        >
